...
import ProcessingNotice from '../utility/ProcessingNotice';
...
const ChatComponent = () => {
  ...
  useEffect(() => {
    if (messageEndRef.current && messages.length > 1) {
      setTimeout(() => {
    ...
    <div className={`${styles.chatContainer}`}>
      <div className={`${styles.chatAndInputBoxContainer}`}>
        <div className={`${styles.chatMessages}`}>
          {showBanner ? <ProcessingNotice /> : null}
          <div ref={messageStartRef} />
          {!showBanner && messages.map((message) => (
            <div
              key={message.msg_id}
        ...
        {processingDocumentAlert.show === true ? <ProcessingAlert message={processingDocumentAlert.message} messageType="warning" /> : (
          <div className={`${styles.inputContainer}`}>
            <input
              ...
              disabled={(isTyping || isDisableInput || showBanner)}
            />
            <button
              type="button"
